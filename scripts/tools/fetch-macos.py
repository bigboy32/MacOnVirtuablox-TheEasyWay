...
class MacOSProduct:
    ...
    def fetchpackages(self, keyword=None):
        packages = self.product['Packages']
        if keyword:
            for item in packages:
                if keyword in item.get("URL"):
                    Filesystem.download_file(item.get("URL"), item.get("Size"))
        else:
            for item in packages:
                Filesystem.download_file(item.get("URL"), item.get("Size"))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            if x == "debugexit": exit()
            catalog_version = versions[list(versions.keys())[int(x)-1]]
            break
        except:
    ...
    product.fetchpackages(keyword="BaseSystem")